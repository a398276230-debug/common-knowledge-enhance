    ...
    public static class Patch_GenerateAndProcessTalkAsync
    {
        ...
        static void Prefix(object talkRequest, List<Pawn> allInvolvedPawns)
        {
            try
            {
                ...
                Log.Message($"[RimTalk Memory] Found {vectorResults.Count} vector candidates (threshold: {lowThreshold:F2})");
                ...
                foreach (var (id, similarity) in vectorResults)
                {
                    ...
                    if (entry != null)
                    {
                        ...
                        {
                            Log.Message($"[RimTalk Memory] Filtered out '{entry.id}' (similarity: {similarity:F2}, importance: {entry.importance:F2}, combined: {score:F2} < threshold: {settings.vectorSimilarityThreshold:F2})");
                        }
                    }
                }
                ...
                Log.Message($"[RimTalk Memory] Successfully injected {finalResults.Count} unique vector knowledge entries into prompt");
                Log.Message($"[RimTalk Memory] Stats: {vectorResults.Count} candidates → {scoredResults.Count} passed combined threshold → {finalResults.Count} final (excluded {keywordMatchedIds.Count} keyword-matched)");
            }
            catch (Exception ex)
            {
